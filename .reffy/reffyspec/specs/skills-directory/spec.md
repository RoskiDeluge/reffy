# skills-directory Specification

## Purpose
Give Reffy's *procedures* the same deterministic, CLI-validated treatment it gives its data. Skills are named, agent-readable task recipes under `.reffy/skills/`, with a managed/unmanaged split: managed skills are CLI-owned and regenerated by `reffy init`, while unmanaged skills are user-authored and never modified. This provides a discoverable, machine-checkable, customizable home for "how to perform one specific task with the Reffy CLI" that the always-loaded `AGENTS.md` instructions route agents toward.
