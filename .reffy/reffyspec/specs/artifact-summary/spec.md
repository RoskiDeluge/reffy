# artifact-summary Specification

## Purpose
TBD - created by archiving change add-reffy-summarize. Update Purpose after archive.
## Requirements
...
The `summarize` command MUST NOT mutate `.reffy/manifest.json` or files in `.reffy/artifacts/`.
...
- **WHEN** `.reffy/artifacts/` has no indexed content to summarize
